...
Back-in-time also known as omniscient debuggers simplify the debugging process by making it easier to follow cause-effect chains from the observable failure back to the causing defect ~\cite{lewis_debugging_2003}\todo{cite}.
...
Even though back-in-time debuggers exist for many object-oriented programming languages\cite{lewis_debugging_2003,hofer_design_2006}\todo{cite: more languages?}, there are none to the best of our knowledge that run on databases and support SQL or SQLScript\footnote{SQLScript is a proprietary SQL extension for stored procedures in SAP HANA~\cite{sqlScript}.}.
...
In the user interface, changes in the values are highlighted in red or green as shown in \cref{tab:diffresult}.
