...
Even though back-in-time debuggers exist for many object-oriented programming languages~\cite{lewis_debugging_2003,hofer_design_2006}\todo{Sort reference numbers (overall in paper if multiple references have been used), cite: more languages?}, there are none to the best of our knowledge that run on databases and support SQL or SQLScript\footnote{SQLScript is a proprietary SQL extension for stored procedures in SAP HANA~\cite{sqlScript}.}.
...
We presented an approach for bringing back-in-time debugging down to SQL and stored procedures and \tool, an implementation of our approach that runs on the SAP HANA in-memory database.
Using \tool, developers can step forwards and backwards through the execution of a stored procedure and query
