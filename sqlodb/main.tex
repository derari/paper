...
The Path tool suite and its Test-Driven Fault Navigation~\cite{perscheid2013} leverages reproducible test cases in order to distribute the tracing overhead over multiple runs depending on developers' needs.\todo{Still very short... please add a bit more related work at least 20 references should be at the end}
...
If a value changed since the \emph{before} step, the old value is shown red; if a value will change until the \emph{after} step, the new value is shown green.
...
We found the overhead of tracing to be consistently between six and seven percent.
...
We also measured the time it takes to reproduce results of each stored procedure.
...
As \cref{tab:measure2} shows, in each case the runtime overhead of time-diff queries is around 25 percent.\todo{Summarize results and emphasize the low overhead that is a requirement for back-in-time debugging.}
