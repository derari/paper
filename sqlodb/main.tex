...
\begin{itemize}
	...
	\item \emph{\SQLextension} is an extension to SQL allowing to submit arbitrary queries against previous states of the database 
		...
		The \tool\, provides a console for developers to submit \SQLextension\, queries which use variables or points in time from the current debug session.
...
We evaluated \tool\, within an internal software development project within SAP. \todo{add some more words to project and number of developers}
First interviews with developers indicate that bugs in stored procedures can be investigated more efficiently with \tool\, than with a regular debugger or other database development tools.
